...
def fit_and_evaluate_adaboost(noise, n_learners=250, train_size=5000, test_size=500):
    ...
    fig = go.Figure(
        layout=go.Layout(
            title=rf"$\textbf{{Error As Function Of The Number Of Fitted Learners, With {noise} Noise}}$"))
    ...
    fig = go.Figure(
        layout=go.Layout(
            title=rf"$\textbf{{The Training Set With A Point Size Proportional To D, With {noise} Noise}}$",
            margin=dict(t=100)))
